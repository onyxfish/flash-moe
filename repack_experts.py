...
def repack_layer(layer_idx, expert_reads, model_path, fds, output_dir, dry_run=False):
    ...
    if dry_run:
        ...
        print(
            f"  Layer {layer_idx:2d}: DRY RUN OK — would write {LAYER_SIZE:,} bytes to {out_path}"
        )
    ...
    for src_fd, src_offset, dst_offset, size in read_plan:
        ...
        if len(data) != size:
            raise IOError(
                f"Short read: expected {size}, got {len(data)} at offset {src_offset}"
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Verify offsets without writing"
    )
    parser.add_argument(
        "--verify-only",
        type=int,
    ...
    if not args.dry_run and args.verify_only is None:
        ...
        if free_bytes < total_bytes:
            print(
                f"WARNING: Not enough free space! Need {needed_gb:.1f} GB but only {free_gb:.1f} GB free."
            )
            print(
                f"Hint: use --layers to process a subset, e.g. --layers 0-{int(free_gb / 3.63) - 1}"
            )
            sys.exit(1)
    ...
    for i, layer_idx in enumerate(layers):
        ...
        if not args.dry_run and bytes_written > 0:
            throughput = (
                bytes_written / elapsed / (1024**3) if elapsed > 0 else float("inf")
            )
            overall_elapsed = time.monotonic() - t_start
            overall_throughput = (
                total_written / overall_elapsed / (1024**3)
    ...
    if not args.dry_run and total_written > 0:
        print(f"\n{'=' * 60}")
        print(
            f"DONE: {total_written:,} bytes ({total_written / 1024**3:.1f} GB) written"
        )
